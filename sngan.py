...
class DCGANGenerator(nn.Sequential):
    def __init__(self, input_shape, bottom_width=4, ch=512, wscale=0.02, hidden_activation='relu',
        ...
        self.append(nn.BatchNormLayer(self.output_shape, layer_name+'/bn_fc1', activation=hidden_activation))
        ...
        self.append(nn.BatchNormLayer(self.output_shape, layer_name+'/bn_deconv1', activation=hidden_activation))

        shape = [o * 2 for o in self.output_shape[2:]]
        self.append(
            nn.TransposedConvolutionalLayer(self.output_shape, ch // 4, 4, shape, layer_name=layer_name + '/deconv2',
                                            padding=1, activation='linear', init=nn.Normal(wscale)))
        self.append(nn.BatchNormLayer(self.output_shape, layer_name + '/bn_deconv2', activation=hidden_activation))

        shape = [o * 2 for o in self.output_shape[2:]]
        self.append(
            nn.TransposedConvolutionalLayer(self.output_shape, ch // 8, 4, shape, layer_name=layer_name + '/deconv3',
                                            padding=1, activation='linear', init=nn.Normal(wscale)))
        self.append(nn.BatchNormLayer(self.output_shape, layer_name + '/bn_deconv3', activation=hidden_activation))
...
def max_singular_value(W, u=None, lp=1):
    ...
    _u = u
    for _ in range(lp):
        _v = _l2normalize(T.dot(_u, W), eps=1e-12)
        _u = _l2normalize(T.dot(_v, W.T), eps=1e-12)
...
class DataManager(nn.DataManager):
    ...
    def load_data(self):
        X_train, _, _, _ = read_data.load_dataset(self.path)
        self.dataset = np.float32(X_train / 255.)
        self.data_size = self.dataset.shape[0]

    def normalize(self, input=None):
        input = input if input is not None else self.dataset
        return (input - .5) * 2.
...
def train_sngan(z_dim=128, image_shape=(3, 32, 32), bs=64, n_iters=int(1e5), n_dis=5):
    ...
    dis = DCGANGenerator(gen.output_shape)
    ...
    for iteration in range(n_iters):
        ...
        for i in range(n_dis):
            batches.__next__()
            training_disc_cost = train_dis()
            if np.isnan(training_disc_cost) or np.isinf(training_disc_cost):
                raise ValueError('Training failed due to NaN cost')
            mon.plot('training disc cost', training_disc_cost)

        if iteration % 500 == 0:
            gen_images = generate()
            mon.imwrite('generated image', gen_images, callback=dm.unnormalize)
